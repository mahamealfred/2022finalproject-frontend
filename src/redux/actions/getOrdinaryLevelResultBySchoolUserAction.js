...
export const getOrdinaryLevelResultsBySchoolUserAction = (id) => async (dispatch) => {
  try {
    ...
    const res = await axios.get(`http://localhost:8000/results/ordinarylevelresults/${id}`, {
      headers: headers,
    });
...
export const resultsRequest = () => {
  return {
    type: GET_ORDINARYRESULTS_BYSCHOOLUSER_REQUEST,
  };
};

export const resultsSuccess = (results) => {
  return {
    type: GET_ORDINARYRESULTS_BYSCHOOLUSER_SUCCESS,
    payload: results,
  };
};
export const resultsFailure = (error) => {
  return {
    type: GET_ORDINARYRESULTS_BYSCHOOLUSER_FAILURE,
    payload: error,
  };
};
