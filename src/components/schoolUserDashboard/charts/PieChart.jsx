...
function PieChartOrd() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      await axios
        .get(
          `http://localhost:8000/results/percentageprimaryresultsbasedongender`,
          {
            headers: headers,
          }
        )
        ...
        .then(function(res) {
          ...
          for (const val of res) {
            ...
            const finalTotal = ((val.total / (resCount * 100)) * 100).toFixed(
              2
            );
  ...
  return (
    <div className="pieChart">
      <span className="pieChartTitle">
        Students Performance Based on Gender P6
      </span>
      <Pie data={data}></Pie>
    </div>
  );
