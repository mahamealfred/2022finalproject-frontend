...
import {getAllOrdinaryResultbyAdminAction} from "../../../../redux/actions/getAllOrdinaryResultbyAdminAction";
...
export default function ListStudent({ openn, ...rest }) {
  ...
  const getAllOrdinaryResult=useSelector((state)=>state.getAllOrdinaryResultbyAdmin);
  ...
  useEffect(()=>{
    async function fetchData(){
     await dispatch(getAllOrdinaryResultbyAdminAction());
  ...
  useEffect(() => {
    async function fetchData() {
      if (!getAllOrdinaryResult.loading) {
        if (getAllOrdinaryResult.results) {
          setResults(getAllOrdinaryResult.results);
    ...
    fetchData();
  }, [getAllOrdinaryResult.results, schoolsState.schools]);
console.log("results:",getAllOrdinaryResult.results.map((p)=>p['student.id']))
  ...
  const searchHandle = async (e) => {
    ...
    try {
      ...
      for (var i = 0; i < getAllOrdinaryResult.results.length; i++) {
        for (var key in getAllOrdinaryResult.results[i]) {
          if (getAllOrdinaryResult.results[i][key] != null) {
            if (
              getAllOrdinaryResult.results[i][key].toString().toLowerCase().indexOf(toSearch) !=
              -1
            ) {
              if (!itemExists(results, getAllOrdinaryResult.results[i]))
                results.push(getAllOrdinaryResult.results[i]);
            }
          }
        }
      }
        ...
        <Box sx={{ minWidth: 1050 }}>
          <Table>
            ...
            <TableBody>
              ...
              <>
                {getAllOrdinaryResult.results.slice(0, limit).map((student) => (
                  <TableRow
                    hover
                    key={student['student.id']}
                    //  selected={selectedStudentIds.indexOf(student.id) !== -1}
